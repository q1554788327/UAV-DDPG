...
class UAVEnvironmentGym(gym.Env):
    def __init__(self, params, num_users=20, max_time=160):
        ...
        # 起点位置
        self.start_position = np.array([0.0, 0.0, self.z_min])  # 起点位置
    ...
    def reset(self, seed=None, options=None):
        ...
        self.uav_state = np.array([0.0, 0.0, self.z_min, 0.0, 0.0, 0.0, 0.0])
        self.current_time = 0
        self.energy = self.max_energy
        ...
        # 初始化历史变量
        self.prev_distance_to_start = 0.0
    ...
    def step(self, action):
        ...
        new_speed = np.clip(target_speed, 0, self.max_horizontal_speed)
        
        # 计算速度分量 (基于偏航角和俯仰角)
        v_x = new_speed * np.cos(new_pitch) * np.sin(new_yaw)
        v_y = new_speed * np.cos(new_pitch) * np.cos(new_yaw)
        ...
        if not self.returning_to_start:
            ...
            for k, psnr in enumerate(user_psnrs):
                if hasattr(self, 'prev_user_psnrs') and self.prev_user_psnrs[k] > 0:
                    ...
                    if improvement > 0:  # PSNR提升
                        psnr_improvement_reward += min(improvement / self.max_psnr * 5.0, 1.0)
            ...
            if len(user_psnrs) > 1:
                ...
                balance_reward = min_max_ratio * 2.5  # 最多+2.5分
                reward += balance_reward
                
                # 额外的平衡奖励：基于PSNR方差的倒数
                psnr_variance = np.var(user_psnrs)
                if psnr_variance > 0:
                    balance_bonus = min(10.0 / (1 + psnr_variance), 1.5)  # 最多+1.5分
            ...
            if distance_to_start > 1.0:
                return_reward = min(150.0 / distance_to_start, 15.0)  # 最大+15分
                reward += return_reward
            else:
                reward += 15.0  # 非常接近起点时给最大奖励
                
            # 7. 返航进步奖励
            progress = self.prev_distance_to_start - distance_to_start
            if progress > 0:  # 只奖励靠近行为
                progress_bonus = min(progress * 6.0, 5.0)  # 每米进步最多+5分
                reward += progress_bonus
        
        # ========== 周期完成奖励 ==========
        if self.at_start_position and self.cycle_step > 0:
            # 8. 周期完成基础奖励
            cycle_completion_reward = 25.0  # 完成周期+25分
            ...
            avg_psnr_bonus = avg_psnr_performance * 15.0  # 最多+15分
            reward += avg_psnr_bonus
            
            # 基于最大PSNR的性能奖励  
            max_psnr_performance = min(cycle_max_psnr / self.max_psnr, 1.0)
            max_psnr_bonus = max_psnr_performance * 10.0  # 最多+10分
            ...
            balance_bonus = balance_factor * 8.0  # 最多+8分
            reward += balance_bonus
            ...
            coverage_bonus = coverage_ratio * 5.0  # 最多+5分
        ...
        if energy_ratio < 0.2:  # 能量不足20%时惩罚
            energy_penalty = (0.2 - energy_ratio) * 5.0  # 最多-1分
            reward -= energy_penalty
        
        # 14. 效率惩罚：鼓励有效运动
        if total_speed < 1.0:  # 速度过慢惩罚
            reward -= 0.2
        
        # ========== 最终奖励处理 ==========
        final_reward = np.clip(reward, -10.0, 60.0)  # 扩大奖励范围适应复杂奖励结构
        ...
        if self.current_time % 25 == 0:
            ...
            print(f"  Energy: {self.energy:.0f}/{self.max_energy:.0f}")
